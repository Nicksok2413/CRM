...
def test_lead_list_view_object_permissions(client, manager1, manager2):
    ...
    # 2.1. ACT (Выполнение действия) для Менеджера 1.

    client.login(username="manager1", password="password")
    response = client.get(url)

    # 3.1. ASSERT (Проверка результата) для Менеджера 1.
    ...
    # 2.2. ACT (Выполнение действия) для Менеджера 2.

    client.login(username="manager2", password="password")
    response = client.get(url)

    # 3.2. ASSERT (Проверка результата) для Менеджера 2.
